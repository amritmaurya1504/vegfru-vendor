...
const StoreDetails = () => {
  ...
  const getStoreById = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/vendor/get-store/${storeId}`, axiosConfig)
      console.log(data)
  ...
  const handleDelete = async (_id) => {
    ...
    try {
      ...
      const { data } = await axios.delete(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/vendor/delete-store/${_id}`, axiosConfig)
      router.push("/store");
    } catch (error) {
  ...
  return (
    <>
      <Sidebar>
        {loader && <div className="flex h-screen items-center justify-center mt-4"><HashLoader color="#22c55e" /></div>}
        {
          singleStore && (
            <div className="bg-gray-100 min-h-screen">
              <div className="flex justify-between px-4 pt-4">
                <h2 className="text-gray-800 font-semibold flex items-center gap-3">
                  <p className="cursor-pointer" onClick={() => router.push("/store")}><Badge variant='outline' colorScheme='green'>
                    Store
                  </Badge></p> <BsArrowBarRight /> {singleStore.storeName}
                </h2>
                ...
                <div class="container mx-auto flex px-5 md:px-24 py-12 md:flex-row flex-col items-center">
                  <div class="lg:flex-grow md:w-1/2 lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center">
                    ...
                    <div class="flex justify-center">
                      ...
                      <button onClick={() => handleDelete(singleStore._id)} class="ml-4 inline-flex text-white bg-red-600 border-0 py-2 px-4 focus:outline-none hover:bg-red-700 font-medium justify-between items-center gap-2">
                        {loaderTwo ? <BeatLoader color="white" /> : (
                          <>
